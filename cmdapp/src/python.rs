...
fn convert_image_to_format(
    image_path: &str,
    out_path: &str,
    format: Option<&str>,          // "svg" (default), "png", "ai"
    ...
    let format = format.unwrap_or("svg").to_lowercase();
    match format.as_str() {
        "svg" => convert_image_to_svg(&input_path, &output_path, config).unwrap(),
        "png" => convert_image_to_png(&input_path, &output_path, config).unwrap(),
        "ai" => convert_image_to_ai(&input_path, &output_path, config).unwrap(),
        _ => return Err(PyException::new_err(format!("Unsupported output format: {}. Supported formats: svg, png, ai", format))),
    }
    Ok(())
}
...
fn vtracer(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(convert_image_to_format, m)?)?;
