...
/// Convert an image file into png file
...
/// Convert an image file into ai file
pub fn convert_image_to_ai(
    input_path: &Path,
    output_path: &Path,
    config: Config,
) -> Result<(), String> {
...
fn read_image(input_path: &Path) -> Result<ColorImage, String> {
    if is_ai_path(input_path) {
        return read_ai_as_image(input_path);
    }

    let img = image::open(input_path)
        .map_err(|_| String::from("No image file found at specified input path"))?
        .to_rgba8();
    let width = img.width() as usize;
    let height = img.height() as usize;

    rgba_to_color_image(img.into_raw(), width, height)
}

fn is_ai_path(input_path: &Path) -> bool {
    input_path
        .extension()
        .and_then(|ext| ext.to_str())
        .map(|ext| ext.eq_ignore_ascii_case("ai"))
        .unwrap_or(false)
}

fn read_ai_as_image(input_path: &Path) -> Result<ColorImage, String> {
    let pdf_data = std::fs::read(input_path).map_err(|err| {
        format!(
            "Failed to read AI file '{}': {}",
            input_path.display(),
            err
        )
    })?;

    let renderer = PdfRenderer::from_bytes(&pdf_data).map_err(|err| {
        format!(
            "Failed to parse AI file '{}'. Only PDF-compatible AI files are supported: {}",
            input_path.display(),
            err
        )
    })?;
    ...
    if renderer.page_count() == 0 {
        return Err(format!(
            "AI file '{}' does not contain any pages. Only PDF-compatible AI files are supported.",
            input_path.display()
        ));
    }

    let page = renderer.render_page(0, 144.0).map_err(|err| {
        format!(
            "Failed to rasterize AI file '{}'. Only PDF-compatible AI files are supported: {}",
            input_path.display(),
            err
        )
    })?;

    rgba_to_color_image(page.pixels, page.width as usize, page.height as usize)
}

fn rgba_to_color_image(pixels: Vec<u8>, width: usize, height: usize) -> Result<ColorImage, String> {
    if width == 0 || height == 0 {
        return Err(String::from("Input image has zero width or height"));
    }
...
fn write_png(svg: SvgFile, output_path: &Path) -> Result<(), String> {
    let svg_str = format!("{}", svg);
    let options = Options::default();
    let tree = Tree::from_str(&svg_str, &options).map_err(|e| e.to_string())?;
    ...
    render(&tree, usvg::FitTo::Original, resvg::tiny_skia::Transform::default(), pixmap.as_mut());
...
fn write_ai(svg: SvgFile, output_path: &Path) -> Result<(), String> {
    // Adobe Illustrator (AI) format is essentially a PDF with an SVG preview
    // For simplicity, we'll create a PDF that contains the SVG content
    let svg_str = format!("{}", svg);
    
    let mut out_file = File::create(output_path).map_err(|_| "Cannot create output file.")?;
    
    // Write a minimal PDF header
    write!(&mut out_file, "%PDF-1.4\n%E2E3CFD3\n").map_err(|e| e.to_string())?;
    
    // Write the SVG as an embedded XObject
    write!(&mut out_file, "1 0 obj\n<</Type/Catalog/Pages 2 0 R>>\nendobj\n").map_err(|e| e.to_string())?;
    write!(&mut out_file, "2 0 obj\n<</Type/Pages/Kids[3 0 R]/Count 1>>\nendobj\n").map_err(|e| e.to_string())?;
    write!(&mut out_file, "3 0 obj\n<</Type/Page/MediaBox[0 0 {} {}]/Contents 4 0 R/Resources<<>>>>\nendobj\n", svg.width, svg.height).map_err(|e| e.to_string())?;
    write!(&mut out_file, "4 0 obj\n<</Length 5 0 R>>\nstream\nq\n1 0 0 1 0 0 cm\n/GS0 gs\n/IM0 Do\nQ\nendstream\nendobj\n").map_err(|e| e.to_string())?;
    write!(&mut out_file, "5 0 obj\n{}\nendobj\n", svg_str.len()).map_err(|e| e.to_string())?;
    write!(&mut out_file, "6 0 obj\n<</Type/XObject/Subtype/Image/Width {}/Height {}/ColorSpace/DeviceRGB/BitsPerComponent 8/Filter/FlateDecode/Length 7 0 R>>\nstream\n{}\nendstream\nendobj\n", svg.width, svg.height, svg_str).map_err(|e| e.to_string())?;
    write!(&mut out_file, "7 0 obj\n{}\nendobj\n", svg_str.len()).map_err(|e| e.to_string())?;
    write!(&mut out_file, "xref\n0 8\n0000000000 65535 f \n0000000009 00000 n \n0000000052 00000 n \n0000000095 00000 n \n0000000202 00000 n \n0000000301 00000 n \n0000000323 00000 n \n0000000440 00000 n \ntrailer\n<</Size 8/Root 1 0 R>>\nstartxref\n{} \n%%EOF\n", svg_str.len() + 450).map_err(|e| e.to_string())?;
